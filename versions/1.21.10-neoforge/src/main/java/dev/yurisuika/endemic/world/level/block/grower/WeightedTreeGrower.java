...
public class WeightedTreeGrower {
    ...
    public static boolean growTree(ServerLevel level, ChunkGenerator chunkGenerator, BlockPos pos, BlockState state, RandomSource random) {
        ...
        ResourceKey<ConfiguredFeature<?, ?>> multiSaplingResourceKey = selectWeightedEntry(filterForAdjacentSaplings(entries, true), random);
        ...
        ResourceKey<ConfiguredFeature<?, ?>> saplingResourceKey = selectWeightedEntry(filterForAdjacentSaplings(entries, false), random);
        if (saplingResourceKey == null) {
            return false;
        } else {
            Holder<ConfiguredFeature<?, ?>> saplingHolder = level.registryAccess().lookupOrThrow(Registries.CONFIGURED_FEATURE).get(saplingResourceKey).orElse(null);
    ...
    public static ResourceKey<ConfiguredFeature<?, ?>> selectWeightedEntry(List<Group.Entry> entries, RandomSource random) {
        ...
        for (Group.Entry entry : entries) {
            if (randomWeight < entry.weight()) {
                selected = ResourceKey.create(Registries.CONFIGURED_FEATURE, entry.feature());
                break;
            } else {
                ...
                if (group.region().dimensions().whitelist().isEmpty() || group.region().dimensions().whitelist().contains(dimension)) {
                    if (group.region().biomes().blacklist().isEmpty() || !group.region().biomes().blacklist().contains(biome)) {
                        if (group.region().biomes().whitelist().isEmpty() || group.region().biomes().whitelist().contains(biome)) {
                            ...
                            double temperatureModifier = calculateModifier(temperature, temperatureConditions.optimum().min(), temperatureConditions.optimum().min(), temperatureConditions.tolerance().min(), temperatureConditions.tolerance().max());
