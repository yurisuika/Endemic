...
public class SaplingManager implements SimpleSynchronousResourceReloadListener {
    ...
    public void onResourceManagerReload(ResourceManager resourceManager) {
        ...
        resourceManager.listResources(DATA_DIRECTORY, path -> path.endsWith(".json")).forEach(location -> {
            ResourceLocation resourceLocation = new ResourceLocation(location.getNamespace(), location.getPath().replace(DATA_DIRECTORY + "/", "").replace(".json", ""));
            try (InputStream stream = resourceManager.getResource(location).getInputStream()) {
                SAPLINGS.put(resourceLocation, Sapling.CODEC.parse(JsonOps.INSTANCE, GSON.fromJson(new InputStreamReader(stream, StandardCharsets.UTF_8), JsonElement.class)).getOrThrow(false, Endemic.LOGGER::error));
            } catch (Exception e) {
                Endemic.LOGGER.error("Failed to parse sapling JSON for {}: {}", resourceLocation, e.getMessage());
            }
        });
    }
