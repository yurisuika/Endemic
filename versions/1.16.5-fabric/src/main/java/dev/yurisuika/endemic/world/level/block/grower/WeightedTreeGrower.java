...
public class WeightedTreeGrower {
    ...
    public static boolean growTree(ServerLevel level, ChunkGenerator chunkGenerator, BlockPos pos, BlockState state, Random random) {
        ...
        ConfiguredFeature<?, ?> multiSaplingConfiguredFeature = selectWeightedEntry(filterForAdjacentSaplings(entries, true), random);
        ...
        ConfiguredFeature<?, ?> saplingConfiguredFeature = selectWeightedEntry(filterForAdjacentSaplings(entries, false), random);
        if (saplingConfiguredFeature == null) {
            return false;
        } else {
    ...
    public static ConfiguredFeature<?, ?> selectWeightedEntry(List<Group.Entry> entries, Random random) {
        if (entries.isEmpty()) {
            return null;
        }
        ...
        for (Group.Entry entry : entries) {
            if (randomWeight < entry.weight()) {
                selected = BuiltinRegistries.CONFIGURED_FEATURE.get(ResourceKey.create(BuiltinRegistries.CONFIGURED_FEATURE.key(), entry.feature()));
                break;
            } else {
    ...
    public static List<Group.Entry> getFilteredEntries(ServerLevel level, BlockPos pos, BlockState state) {
        ...
        ResourceLocation biome = level.registryAccess().registryOrThrow(Registry.BIOME_REGISTRY).getKey(level.getBiome(pos));
        int elevation = pos.getY();
        int brightness = level.getRawBrightness(pos, 0);
        float temperature = level.getBiome(pos).getTemperature(pos);
        float downfall = level.getBiome(pos).getDownfall();
